...
DATA_DIR = "data"
WORD_FREQ_FILE = os.path.join(DATA_DIR, "Lexique_FR2.tsv")
SEEN_WORDS = os.path.join(DATA_DIR, "seen_words_motus.json")
PATTERN_MATRIX_FILE = os.path.join(DATA_DIR, "pattern_matrix_motus.npy")
BEST_STARTERS_FILE = os.path.join(DATA_DIR, "best_starters.json")
EXCLUDED = ['carien', 'aitres', 'atresie', 'martins']
...
#
def get_word_list(pattern_start):
    nb_lettres = len(pattern_start)
    if nb_lettres == 2:
        nb_lettres = int(pattern_start[1])
    reg_pattern = re.compile('{}[a-z]{{{}}}'.format(pattern_start[0], nb_lettres - 1))
    freqs = []
    word_freq_raw = dict()
    with open(WORD_FREQ_FILE, 'r', encoding='utf-8') as fp:
        for lines in fp.readlines()[1:]:
            line_split = lines.split('\t')
            if int(line_split[3]) == nb_lettres:
                word = remove_accents(line_split[0])
                if word not in EXCLUDED:
                    if reg_pattern.match(word):
                        freq = np.mean([float(s) for s in line_split[1:2]]) / 1e6
                        freqs.append(freq)
...
def generate_best_starter(look_two_ahead=False,
                          optimize_for_uniform_distribution=False,
                          purely_maximize_information=False,
                          regenerate=False):
    best_starters = dict()
    choice_config = dict()
    choice_config['look_two_ahead'] = look_two_ahead
    choice_config['optimize_for_uniform_distribution'] = optimize_for_uniform_distribution
    choice_config['purely_maximize_information'] = purely_maximize_information
    if os.path.exists(BEST_STARTERS_FILE):
        with open(BEST_STARTERS_FILE) as f:
            best_starters = json.load(f)
    k_to_exclude = []
    for k, v in best_starters.items():
        if v in EXCLUDED:
            k_to_exclude.append(k)
    for k in k_to_exclude:
        del best_starters[k]
    if regenerate:
        for nl in range(5, 10):
            for l in range(26):
                letter = chr(97 + l)
                k = '{}{}'.format(letter, nl)
                if not k in best_starters:
                    init()
                    get_word_list(k)
                    if len(WORD_LIST) == 0:
                        print('{}: No words'.format(k))
                        continue
                    all_words = WORD_LIST
                    priors = PRIORS
                    best_guess = optimal_guess(
                        all_words, all_words, priors,
                        **choice_config
                    )
                    print('{}: {}'.format(k, best_guess))
                    best_starters[k] = best_guess

        with open(BEST_STARTERS_FILE, 'w') as fp:
            json.dump(best_starters, fp)
    global BEST_STARTERS
...
def generate_pattern_matrix(words1, words2):
    ...
    pattern_matrix = np.dot(
        full_pattern_matrix,
        (3 ** np.arange(nl)).astype(np.uint16)
    )
...
def get_word_buckets(guess, possible_words):
    nl = len(possible_words[0])
    buckets = [[] for x in range(3 ** nl)]
...
def get_pattern_distributions(allowed_words, possible_words, weights):
    ...
    n = len(allowed_words)
    nl = len(allowed_words[0])
    distributions = np.zeros((n, 3 ** nl))
    n_range = np.arange(n)
...
def get_average_second_step_entropies(first_guesses, allowed_second_guesses, possible_words, priors):
    ...
    for first_guess, dist in ProgressDisplay(list(zip(first_guesses, distributions)),
                                             leave=False,
...
def get_expected_scores(allowed_words, possible_words, priors,
    ...
    allowed_second_guesses = WORD_LIST
    expected_scores += 1  # Push up the rest
    for i in ProgressDisplay(sorted_indices[:n_top_candidates_for_two_step], leave=False, ascii=True):
...
def optimal_guess(allowed_words, possible_words, priors,
    ...
    if optimize_for_uniform_distribution:
        ...
        if print_best_result:
            words_expected_score = dict(zip(allowed_words, expected_scores))
            words_expected_score_sorted = {k: v for k, v in
                                           sorted(words_expected_score.items(),
                                                  key=lambda ele: ele[1])}
            for k in list(words_expected_score_sorted.keys())[:10]:
                print('{} -> {}'.format(k, words_expected_score_sorted[k]))
...
#     guess_to_score = {}
#     guess_to_dist = {}
#     for row in ProgressDisplay(double_ents[:n_top_candidates], ascii=True):
#         first_guess = row[0]
...
def find_smallest_second_guess_buckets(n_top_picks=100):
    ...
    avg_ts_buckets = []
    for first_guess, dist in ProgressDisplay(list(zip(top_picks, top_dists)), ascii=True):
...
def get_optimal_second_guess_map(first_guess, n_top_picks=10, regenerate=False):
    # if os.path.exists(SECOND_GUESS_MAP_FILE):
    ...
    nl = len(all_words[0])
    sgm = [""] * 3 ** nl
    wordle_answers = all_words
    # wordle_answers = get_word_list(short=True)
    # priors = get_true_wordle_prior()

    buckets = get_word_buckets(first_guess, wordle_answers)
    for pattern, bucket in ProgressDisplay(list(enumerate(buckets)), position=0, leave=False, ascii=True):
...
def gather_entropy_to_score_data(first_guess, priors=None):
    words = get_word_list()
    answers = get_word_list(short=True)
    if priors is None:
        priors = PRIORS
...
def get_starter():
    ...
    priors = PRIORS
    choice_config = dict()
    choice_config['look_two_ahead'] = False
    choice_config['optimize_for_uniform_distribution'] = False
    choice_config['purely_maximize_information'] = True
...
def simulate_games(starter=None,
                   first_guess=None,
                   priors=None,
                   look_two_ahead=False,
                   optimize_for_uniform_distribution=False,
                   ...
                   quiet=False,
                   real_game=False,
                   always_take_suggested=False,
                   ):
    if not starter:
        starter = input('Starter is: ').lower()
    nl = len(starter)
    if nl >= 2 and nl <= 3:
        nl = int(starter[1:])
        starter_key = starter
        starter = starter[0] * nl
    else:
        starter_key = '{}{}'.format(starter[0], nl)

    get_word_list(starter)
    all_words = WORD_LIST
    short_word_list = all_words
    possible_words = short_word_list
    ...
    if first_guess is None:
        if starter_key in BEST_STARTERS:
            first_guess = BEST_STARTERS[starter_key]
        else:
            first_guess = optimal_guess(
                all_words, all_words, priors,
                **choice_config)
            BEST_STARTERS[starter_key] = first_guess
            with open(BEST_STARTERS_FILE, 'w') as fp:
                json.dump(BEST_STARTERS, fp)
    ...
    seen = set()
    if os.path.exists(SEEN_WORDS):
        with open(SEEN_WORDS, 'r') as fp:
            seen_dict = json.load(fp)
    else:
        seen_dict = dict()
    seen = set(seen_dict.values())
    ...
    if real_game:
        ...
        # value_proba = get_guess_values_array(all_words, possibilities, priors, look_two_ahead)
        score = 0
        if first_guess:
            print('Your first guess should be: {}'.format(first_guess))
        guess = first_guess
        pattern = 0

        pattern_max = (3 ** nl) - 1
        while pattern != pattern_max:
            if not always_take_suggested:
                guess = input('What is your {} guess? '.format('first' if score == 0 else 'next'))
            pattern_string = ''
            ok = len(pattern_string) == nl
            while not ok:
                if len(pattern_string) > 0 and pattern_string != 'new':
                    print('Wrong number of character, try again')
                elif pattern_string == 'new':
                    guess = input('What is your {} guess? '.format('first' if score == 0 else 'next'))
                pattern_string = input('\nInput the pattern: (gray:0, yellow:1, green:2)')
                ok = (len(pattern_string) == nl and pattern_string != 'new') or pattern_string == 'ok'
            if pattern_string == 'ok':
                pattern = pattern_max
            else:
                ...
                print('{} possibilities\nThe 10 first are:'.format(possibility_count))
                freq_total = sum([freq_list[ww] for ww in possibilities])
                freq_possibilities = [(w, freq_list[w] / freq_total) for w in possibilities]
                freq_possibilities.sort(key=lambda y: -y[1])
                for i in range(min(10, possibility_count)):
                    print('{} ({:.3%})'.format(freq_possibilities[i][0], freq_possibilities[i][1]))
                possibility_counts.append(possibility_count)
                score += 1
                guess = get_next_guess(guesses, patterns, possibilities)
                print('The next guess should be: {}'.format(guess))

        d = datetime.date.today()
        while d.strftime('%Y-%m-%d') in seen_dict:
            d -= datetime.timedelta(1)
        seen_dict[d.strftime('%Y-%m-%d')] = guess
        seen_dict = {key: value for key, value in sorted(seen_dict.items(), key=lambda item: item[0])}
        with open(SEEN_WORDS, 'w') as fp:
            json.dump(seen_dict, fp)
        print('Congrats: final answer was {} (score:{})'.format(guess, score))
        return True, True
    else:
        ...
        game_results = []
        for answer in ProgressDisplay(test_set,
                                      leave=False,
...
if __name__ == "__main__" and True:
    again = True
    regenerate = False
    generate_best_starter(regenerate=regenerate)
    while again:
        init()
        results, decision_map = simulate_games(first_guess=None,
                                               priors=None,
                                               optimize_for_uniform_distribution=False,
                                               real_game=True,
                                               exclude_seen_words=False,
                                               shuffle=True,
                                               always_take_suggested=True)
        if input('again ? [y|n]') != 'y':
            again = False

# re_only_letter = re.compile('[a-z]+')
# freqs_total = [0, 0, 0, 0, 0, 0]
# word_list = dict()
# with open('data/Fre.Freq.2.tsv', encoding='utf-8') as f:
...
#             freqs_total[i] += val
#         try:
#             word = remove_accents(l_s[0]).lower().strip()
#             print(word)
#         except:
#             continue
#         if re_only_letter.fullmatch(word) and word not in word_list:
#             word_list[word] = freqs
# word_freq = dict()
# for w in word_list:
#     for i in range(6):
#         word_list[w][i] /= freqs_total[i]
#         word_freq[w] = np.mean(word_list[w])
# with open('data/worldlex_fr.json', 'w') as fp:
